                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        @foreach($courses as $course)
                            <tr>
                                ...
                                <td>
                                    @if($course->image)
                                        <img src="{{ asset('uploads/courses/' . $course->image) }}" width="50" alt="Course Image">
                                    @endif
                                </td>
                                <td class="text-center">
                                    <a href="{{ route('online-courses.edit', $course->id) }}" class="btn btn-info btn-sm"><i class="fa fa-edit"></i></a>
                                    <form action="{{ route('online-courses.destroy', $course->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure?');">
                                        @csrf
                                        @method('DELETE')
